/*
  ...
  gengetopt -C -i options.ggo -f cmdline.c 
...
const char *gengetopt_args_info_purpose = "Relay RTL TCP data to multiple clients.";
...
static int
cmdline_c_internal (int argc, char **argv, struct gengetopt_args_info *args_info,
                        struct cmdline_c_params *params, const char *additional_error);
...
void
cmdline_c_print_version (void)
{
  printf ("%s %s\n",
     (strlen(CMDLINE_C_PACKAGE_NAME) ? CMDLINE_C_PACKAGE_NAME : CMDLINE_C_PACKAGE),
     CMDLINE_C_VERSION);
...
void
cmdline_c_init (struct gengetopt_args_info *args_info)
{
...
void
cmdline_c_params_init(struct cmdline_c_params *params)
{
  if (params)
    { 
      params->override = 0;
...
cmdline_c_params_create(void)
{
  struct cmdline_c_params *params = 
    (struct cmdline_c_params *)malloc(sizeof(struct cmdline_c_params));
...
static void
cmdline_c_release (struct gengetopt_args_info *args_info)
{
...
int
cmdline_c_dump(FILE *outfile, struct gengetopt_args_info *args_info)
{
  int i = 0;
...
int
cmdline_c_file_save(const char *filename, struct gengetopt_args_info *args_info)
{
  FILE *outfile;
  int i = 0;
  ...
  if (!outfile)
    {
      fprintf (stderr, "%s: cannot open file for writing: %s\n", CMDLINE_C_PACKAGE, filename);
      return EXIT_FAILURE;
    }

  i = cmdline_c_dump(outfile, args_info);
  fclose (outfile);
...
void
cmdline_c_free (struct gengetopt_args_info *args_info)
{
  cmdline_c_release (args_info);
}
...
int
cmdline_c (int argc, char **argv, struct gengetopt_args_info *args_info)
{
  return cmdline_c2 (argc, argv, args_info, 0, 1, 1);
}

int
cmdline_c_ext (int argc, char **argv, struct gengetopt_args_info *args_info,
                   struct cmdline_c_params *params)
{
  int result;
  result = cmdline_c_internal (argc, argv, args_info, params, 0);

  if (result == EXIT_FAILURE)
    {
      cmdline_c_free (args_info);
      exit (EXIT_FAILURE);
    }
...
int
cmdline_c2 (int argc, char **argv, struct gengetopt_args_info *args_info, int override, int initialize, int check_required)
{
  int result;
  struct cmdline_c_params params;
...
int
cmdline_c_required (struct gengetopt_args_info *args_info, const char *prog_name)
{
  FIX_UNUSED (args_info);
 ...
 * @param check_ambiguity @see cmdline_c_params.check_ambiguity
 * @param override @see cmdline_c_params.override
                        ...
                        struct cmdline_c_params *params, const char *additional_error)
{
  ...
  if (initialize)
    cmdline_c_init (args_info);

  cmdline_c_init (&local_args_info);
  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case 'V':	/* Print version and exit.  */
          cmdline_c_print_version ();
          cmdline_c_free (&local_args_info);
          exit (EXIT_SUCCESS);
        ...
        case 'h':	/* rtl_tcp host address.  */
          ...
          break;
        case 'l':	/* Listening port for client connections.  */
        ...
        default:	/* bug: option not considered.  */
          fprintf (stderr, "%s: option unknown: %c%s\n", CMDLINE_C_PACKAGE, c, (additional_error ? additional_error : ""));
          abort ();
        } /* switch */
    } /* while */
  ...
  cmdline_c_release (&local_args_info);
...
static int
_cmdline_c_configfile (const char *filename, int *my_argc)
{
  FILE* file;
  ...
  while ((fgets(linebuf, CONFIG_FILE_LINE_SIZE, file)) != 0)
      ...
      if (len > (CONFIG_FILE_LINE_BUFFER_SIZE-1))
        {
          fprintf (stderr, "%s:%s:%d: Line too long in configuration file\n",
                   CMDLINE_C_PACKAGE, filename, line_num);
          result = EXIT_FAILURE;
          break;
        }
    ...
    noarg:
      if (!strcmp(fopt,"include")) {
        if (farg && *farg) {
          result = _cmdline_c_configfile(farg, my_argc);
        } else {
                           ...
                           int override, int initialize, int check_required)
{
  ...
  return cmdline_c_config_file (filename, args_info, &params);
}

int
cmdline_c_config_file (const char *filename,
                           struct gengetopt_args_info *args_info,
                           struct cmdline_c_params *params)
{
  int i, result;
  ...
  cmd_line_list_tmp->next = cmd_line_list;
  cmd_line_list = cmd_line_list_tmp;
  cmd_line_list->string_arg = gengetopt_strdup (CMDLINE_C_PACKAGE);

  result = _cmdline_c_configfile(filename, &my_argc);
  ...
  if (result != EXIT_FAILURE) {
    ...
    result =
      cmdline_c_internal (my_argc, my_argv_arg, args_info,
                              params,
                              additional_error);
